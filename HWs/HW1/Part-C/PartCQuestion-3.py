...
if __name__ == "__main__":
    ...
    spark = SparkSession\
        .builder\
        .appName("CS-744-Assignment1-PartC-3")\
...
# Return (u'287144', <pyspark.resultiterable.ResultIterable object at 0x7f3dafc46550>) 
    ...
    links = lines.map(lambda line: (line.split("\t")[0], line.split("\t")[1])).distinct().groupByKey().partitionBy(partitions).cache()
...
# 10 interation for calculating the Rank
    ...
    for iteration in range(int(sys.argv[2])):
        ...
        Contributes = links.join(ranks)\
                    .flatMap(lambda line: computeContributes(line[1][0], line[1][1])).cache()
        
        ranks = Contributes.reduceByKey(add).mapValues(lambda rank: rank*0.85 + 0.15).cache()
